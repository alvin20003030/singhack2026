...
def _format_analytics_context(analytics_context: dict) -> str:
    """Format derived analytics as authoritative, read-only chat evidence."""
    if not analytics_context:
        return "DERIVED ANALYTICS: No analytics snapshot was supplied. Do not invent metrics."

    def records(section: str) -> list:
        value = analytics_context.get(section, [])
        return value if isinstance(value, list) else []

    lines = [
        "DERIVED ANALYTICS (authoritative values calculated by analytics.py for the deep dive):",
        "Use these values as-is. Do not recalculate, estimate, or replace them with general knowledge.",
    ]
    for section in ["portfolio_summary", "ltv", "liquidity", "mandate_drift", "lookthrough_exposure", "monitoring_alerts"]:
        lines.append(f"\n{section.upper()}:")
    ...
    tax_data = analytics_context.get("tax_optimization", {})
    lines.append("\nTAX_OPTIMIZATION:")
    lines.append(f"- {tax_data}")
    return "\n".join(lines)
...
def generate_rm_chat_response(
    ...
    """Answer an RM chat prompt using only the selected client's grounded context."""
    context = "\n\n".join([
        _format_event_log(events_df),
        _format_client_context(client_detail),
        _format_holdings(holdings_df),
        _format_attributions(attributions),
        _format_analytics_context(analytics_context or {}),
    ])
    prompt = f"""You are supporting Priscilla Ong in {mode}.

RM QUESTION:
{user_message}

Answer using the client context and DERIVED ANALYTICS above. Treat DERIVED ANALYTICS as
the source of truth for all figures, alerts, ratios, exposure, tax totals, and drift.
Do not calculate substitute values from raw data. For any 2026 event explanation, cite the exact
event date and description from EVENT LOG. If the data does not support a claim, say so.
Keep all investment actions as editable drafts for RM review; never represent them as
approved advice. Use plain, senior-client-friendly language when drafting outreach."""
    if _gemini_available:
        return _call_gemini(prompt, context)

    event_lines = [
        f"- {event['event_date']}: {event['description']}"
        for event in events_df.to_dict("records")[:4]
    ]
    return (
        f"**RM draft response ({mode})**\n\n{user_message}\n\n"
        "I would anchor the discussion in the analytics.py-derived metrics, documented portfolio data, and RM notes. "
        "The event-log references available for this draft are:\n"
        + "\n".join(event_lines)
        + "\n\n[RM REVIEW REQUIRED] Confirm the figures, tax treatment, and final wording before use."
    )
